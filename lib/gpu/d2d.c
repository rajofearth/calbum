...
{
    (void) r;
    if (!txt->d2d_rtv || !format)
        return;

    D2D1_COLOR_F c = {color[0], color[1], color[2], color[3]};
    txt->d2d_brush->lpVtbl->SetColor(txt->d2d_brush, &c);

    D2D1_RECT_F layoutRect = {x, y, x + w, y + h};
    ID2D1RenderTarget_DrawText(txt->d2d_rtv, text, (UINT32) wcslen(text), format, &layoutRect,
                               (ID2D1Brush *) txt->d2d_brush, D2D1_DRAW_TEXT_OPTIONS_NONE,
...
void r_draw_text(TextState *txt, GpuState *r, const wchar_t *text, float x, float y, float w, float h)
{
    ...
    r_draw_text_ext(txt, r, text, x, y, w, h, txt->dwrite_format, white);
}
...
{
    (void) r;
    if (!txt->d2d_rtv || !txt->dwrite_factory || !format)
        return;

    // Fast path: cached layouts for single-character icon glyphs
    if (wcslen(text) == 1 && txt->dwrite_format_icons == format)
    {
        struct IDWriteTextLayout *layout = NULL;
        if (text[0] == 0xE72B)
        {
            layout = txt->layout_back;
        }
        else if (text[0] == 0xE946)
        {
            layout = txt->layout_info;
        }
        ...
        if (layout)
        {
            ...
            txt->d2d_rtv->lpVtbl->DrawTextLayout(txt->d2d_rtv, origin, layout, (ID2D1Brush *) txt->d2d_brush,
    ...
    struct IDWriteTextLayout *layout = NULL;
    HRESULT hr = txt->dwrite_factory->lpVtbl->CreateTextLayout(txt->dwrite_factory, text, (UINT32) wcslen(text), format,
                                                               w, h, &layout);
...
{
    if (!txt->dwrite_factory || !format || !text)
        return 0.0F;
