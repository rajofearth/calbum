...
int r_load_full_image(AppState *s, const wchar_t *path)
{
    ...
    if (FAILED(s->d3d_device->lpVtbl->CreateTexture2D(s->d3d_device, &desc, &init_data, &new_slot->texture)))
        return 0;

    if (FAILED(s->d3d_device->lpVtbl->CreateShaderResourceView(s->d3d_device, (ID3D11Resource *) new_slot->texture,
                                                               NULL, &new_slot->srv)))
    {
