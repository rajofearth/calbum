...
- **Test-Code Synchronization (Zero-Drift)**: ALWAYS keep tests up to date and in sync with the codebase. If a function or structure is refactored, modified, or deleted, its corresponding tests MUST be updated in the exact same commit/step. You must verify that the test suite compiles and passes after every significant change. The test suite must always act as a rigid gatekeeper. Stale tests give a false sense of security and are considered broken. Failing to fix failing tests is a violation of this philosophy.
