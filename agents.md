...
- **Test-Code Synchronization (Zero-Drift)**: Tests must evolve with the code. If a function is refactored or deleted, its tests must be updated immediately. The test suite must always compile and act as a rigid gatekeeper. Stale tests give a false sense of security and are considered broken.
